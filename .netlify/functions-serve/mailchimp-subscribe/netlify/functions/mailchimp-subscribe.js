...
var mailchimp_subscribe_exports = {};
__export(mailchimp_subscribe_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(mailchimp_subscribe_exports);
...
var handler = async (event, context) => {
  ...
  try {
    ...
    if (!apiKey || !serverId || !listId) {
      console.error(
        "Les variables d'environnement Mailchimp ne sont pas configur\xE9es"
      );
      return {
        statusCode: 500,
        body: JSON.stringify({ error: "Configuration du serveur incompl\xE8te" })
      };
    }
    const subscriberHash = import_crypto.default.createHash("md5").update(email.toLowerCase()).digest("hex");
    const tags = [source];
    const data = {
      email_address: email,
      status: "pending",
      // 'pending' pour double opt-in, 'subscribed' pour inscription directe
      merge_fields: {
        FNAME: firstName,
        LNAME: lastName
      },
    ...
    const response = await (0, import_node_fetch.default)(url, {
      method: "PUT",
      // PUT pour mettre à jour ou créer
      headers: {
    ...
    if (response.ok) {
      return {
        ...
        body: JSON.stringify({
          status: "success",
          message: "Inscription en cours, veuillez v\xE9rifier votre bo\xEEte mail"
        })
      };
    } else {
