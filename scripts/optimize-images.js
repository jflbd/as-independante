...
async function optimizeImage(filePath) {
  try {
    ...
    if (ext === ".png") {
      // Qualité augmentée à 90% pour les PNG et compression moins agressive
      await img.png({ quality: 90, compressionLevel: 7 }).toFile(tempPath);
    } else {
      // Qualité augmentée à 90% pour les JPEG
      await img.jpeg({ quality: 90, mozjpeg: true }).toFile(tempPath);
    }
    ...
    if (optimizedStats.size < originalStats.size) {
      ...
      console.log(
        `✓ ${path.basename(filePath)} - Réduction: ${reduction.toFixed(2)}% (${(
          originalStats.size / 1024
        ).toFixed(2)}KB → ${(optimizedStats.size / 1024).toFixed(2)}KB)`
      );
    } else {
